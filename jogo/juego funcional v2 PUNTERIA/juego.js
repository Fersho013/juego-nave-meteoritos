...
function dibujarTextoCentral(texto, subtitulo = '') {
  ...
  if (estado === 'inicio') {
    ...
    ctx.fillText('ESPACIO: Disparar', canvas.width / 2, canvas.height / 2 + 150);
...
if (mousePresionado) {
  tiempoDisparo++;
  if (tiempoDisparo >= 10) { // cada 10 frames (ajustable)
    disparar();
    tiempoDisparo = 0;
  }
} else {
  tiempoDisparo = 10; // para disparar inmediatamente al hacer clic
}

  // Actualizar inmunidad y parpadeo
  if (nave.inmune) {
  ...
  if (contadorMeteoritos % frecuenciaMeteoritos === 0 && !jefe) {
    ...
    vx = (dx / distancia) * velocidadMeteoritos;
    vy = (dy / distancia) * velocidadMeteoritos;
  ...
  // Aparecer jefe si puntaje >= 50 y no existe ya
  if (puntaje >= 50 && !jefe) {
    jefe = {
      x: canvas.width / 2,
      y: 80,
      ...
      dirX: 1,
      dirY: 1,
      disparoCooldown: 10 // frames para disparar
    };
  }
  ...
  meteoritos.forEach((m, i) => {
    ...
    // Eliminar meteoritos fuera pantalla
    if (
      m.x < -30 || m.x > canvas.width + 30 ||
      m.y < -30 || m.y > canvas.height + 30
    ) {
      meteoritos.splice(i, 1);
      puntaje++;
    }
  ...
  if (jefe) {
    ...
    // Disparo jefe
    contadorDisparoJefe++;
    if (contadorDisparoJefe >= jefe.disparoCooldown) {
      contadorDisparoJefe = 0;
...
function dibujarJefe() {
  ...
  // Dibujamos como una nave negra simple, apuntando hacia la nave
  const angle = Math.atan2(nave.y - jefe.y, nave.x - jefe.x);
  ctx.rotate(angle);
  ...
  // Dibujar barra de vida encima del jefe
  ctx.fillStyle = 'red';
  ctx.fillRect(jefe.x - 40, jefe.y - jefe.height/2 - 10, 80, 8);
  ctx.fillStyle = 'lime';
  ctx.fillRect(jefe.x - 40, jefe.y - jefe.height/2 - 10, 80 * (jefe.vidas / 20), 8);
}
