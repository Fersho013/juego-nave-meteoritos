...
export const superBossState = {
    active: false,
    arena: false, // true mientras está en zona azul/roja
    phase: 1, // 1: cañones, 2: compuerta, 3: lasers/escudo
    x: 0, y: 0, w: 0, h: 0,
    hp: 8000, maxHp: 8000,
    shield: true,
    cannons: [], // {x,y, hp,maxHp, lastShot, alive}
    gate: null, // {x,y,w,h, hp,maxHp, open, lastSpawn}
    lasersActive: false,
    laserTimer: 0,
    shieldTimer: 0,
    destroyed: false,
    explosionTimer: 0,
    modifierPickup: null // {x,y, active}
};
...
export function updateSuperBoss({ canvas, nave, bullets, enemyBullets, enemies, particles, floatingTexts, fxState, createExplosion, spawnDebris, hudState, combatState, dropPickup, frameCount }) {
    ...
    // Si destruido, manejar animación
    if (superBossState.hp <= 0 && !superBossState.destroyed) {
        superBossState.destroyed = true;
        superBossState.explosionTimer = 90;
        // otorgar modifier pickup
        superBossState.modifierPickup = { x: canvas.width / 2, y: canvas.height / 2, vy: 1.2, active: true };
    ...
    if (superBossState.destroyed) {
        ...
        if (superBossState.explosionTimer <= 0) {
            // Dejar pickup y limpiar boss pero mantener arena hasta recoger pickup o timeout
            // El cleanup real se hace al recoger pickup o al salir
        }
        // Actualizar pickup flotante
        if (superBossState.modifierPickup?.active) {
            const mp = superBossState.modifierPickup;
            mp.y += mp.vy;
            if (mp.y > canvas.height - 20) mp.vy *= -1;
    ...
    if (superBossState.phase === 1) {
        ...
        // spawn enemigos desde compuerta cada ~900ms
        if (Date.now() - g.lastSpawn > 850) {
...
export function drawSuperBoss(ctx, frameCount) {
    ...
    // Si destruido, solo dibujar restos breves
    if (sb.destroyed) {
        ctx.save();
